...
import numpy as np
np.random.seed(0)
from util_depth import phrase2vec, onehot
import os.path
import cv2
import math
import sys
...
from util_depth import read_img
from util import multispafea
...
class SpatialDataset(Dataset):
    ...
    def __init__(self, split, load_image, args):
        ...
        for idx1,img in enumerate (json.load(open(args.datapath))):
                for annot in img['annotations']:
                    annot['url'] = img['url']
                    annot['height'] = img['height']
                    annot['width'] = img['width']
                    annot['subject']['depth'] = torch.Tensor(np.array(annot['subject']['depth'], np.float32))
                    annot['object']['depth'] = torch.Tensor(np.array(annot['object']['depth'], np.float32))
        ...
        trainset_num = math.ceil(len(self.annotations)/10*8)
        valid_test_num = math.ceil(len(self.annotations)/10*1)
...
def create_dataloader_depth(split, load_image, args):
